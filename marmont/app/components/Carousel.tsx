...
function DefaultIcon() {
  return (
    <svg className={styles.carouselIcon} viewBox="0 0 24 24" aria-hidden="true">
...
function CarouselItem({ item, index, itemWidth, round, trackItemOffset, x, transition }: CarouselItemProps) {
  ...
  const iconContent = typeof item.icon === 'string' && item.icon.trim().length > 0
    ? <span className={styles.carouselEmoji}>{item.icon}</span>
  ...
  return (
    ...
    >
      <div className={`${styles.carouselItemHeader} ${round ? styles.round : ''}`}>
        <span className={styles.carouselIconContainer}>{iconContent}</span>
      </div>
...
export default function Carousel({
  ...
  const [measuredWidth, setMeasuredWidth] = useState<number>(baseWidth ?? 320);
  const resolvedWidth = baseWidth ?? measuredWidth;
  const itemWidth = resolvedWidth - containerPadding * 2;
  ...
  useEffect(() => {
    if (baseWidth !== undefined) return undefined;
